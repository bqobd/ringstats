...
import { fetchTournaments, fetchTournamentDetails } from '../api';
...
export default function TournamentResults() {
  ...
  const loadTournaments = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Kunde inte ladda tävlingsresultat');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center space-x-2 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Tävlingsresultat</h1>
      </div>
      ...
      {!loading && tournaments.length === 0 && !error && (
        <div className="text-center py-12">
          <p className="text-gray-600">Inga tävlingsresultat tillgängliga</p>
        </div>
      )}
    </div>
  );
